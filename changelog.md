...
### Добавлено
- **Валидация Minecraft никнеймов** - добавлена строгая проверка никнеймов согласно правилам Mojang
- **Подробные сообщения об ошибках** - пользователи получают детальную информацию о требованиях к никнейму
- **Утилиты валидации** - создан модуль `minecraftValidation.ts` с функциями проверки и форматирования ошибок

### Изменено
- **Улучшенная валидация при подаче заявления** - никнеймы теперь проверяются по регулярному выражению `^[a-zA-Z0-9_]{3,16}$`
- **Требования к никнейму**: длина 3-16 символов, разрешены только латинские буквы, цифры и подчеркивания
- **Обработка ошибок** - более информативные сообщения с примерами корректных никнеймов

## [1.0.2] - 2025-09-06

### Исправлено
- **Критическая ошибка доступа при подаче заявлений** - исправлена проблема, когда новые пользователи получали ошибку "Access denied" при попытке подать заявление
- **Глобальный adminMiddleware** - убран глобальный middleware из adminController, который неправильно перехватывал все текстовые сообщения
- **Селективная защита админских функций** - adminMiddleware теперь применяется только к конкретным callback query обработчикам, требующим админских прав
- **Обработка текстовых сообщений** - восстановлена корректная работа обработчика `on('message:text')` для обычных пользователей

### Улучшено
- **Архитектура middleware** - более точное применение проверок прав доступа только там, где это необходимо
- **Пользовательский опыт** - устранены препятствия для новых пользователей при подаче заявлений
