  ...
  async getMainKeyboard(userId?: number): Promise<Keyboard> {
    ...
    if (userId) {
      try {
        ...
        } else if (user && user.role === RoleManager.ROLES.APPLICANT) {
          // Для пользователей с ролью APPLICANT (подавших заявку)
          keyboard.text("📊 Статус заявки").row();
          keyboard.text("ℹ️ Помощь").text("📋 О сервере").row();
        } else if (user && user.role === RoleManager.ROLES.VISITOR) {
          // Для пользователей с ролью VISITOR (начавших взаимодействие)
          keyboard.text("📝 Подать заявку").row();
          keyboard.text("ℹ️ Помощь").text("📋 О сервере").row();
        } else {
          // Для незарегистрированных пользователей показываем только базовые кнопки
          keyboard.text("📝 Подать заявку").row();
          keyboard.text("ℹ️ Помощь").text("📋 О сервере").row();
        }
      } catch (error) {
        logger.error("Ошибка при проверке прав пользователя:", error);
        // В случае ошибки показываем базовые кнопки для незарегистрированных
        keyboard.text("📝 Подать заявку").row();
        keyboard.text("ℹ️ Помощь").text("📋 О сервере").row();
      }
    } else {
      // Для неавторизованных пользователей показываем только базовые кнопки
      keyboard.text("📝 Подать заявку").row();
      keyboard.text("ℹ️ Помощь").text("📋 О сервере").row();
    }
